...
// drives 14 panel outputs accordingly. PD1 is flashed via SWIO by the ESP when the
...
//   - Only RX is enabled; PD0 stays a reserved input (no USART TX function driven).
...
//   b0[0..6] -> PD4,PD5,PD6,PD7,PA1,PA2,PC0
//   b1[0..6] -> PC3,PC4,PC5,PC6,PC7,PD2,PD3
//   b2       -> reserved (PD0,PC1,PC2 are inputs for now) — ignored.
#define FRAME_SYNC 0xAA
#define BAUD 4800

// 14 active outputs, packed bit i (i=0..13) -> outputs[i].
static const uint8_t outputs[14] = {
  PD4, PD5, PD6, PD7, PA1, PA2, PC0, // b0[0..6]
  PC3, PC4, PC5, PC6, PC7, PD2, PD3  // b1[0..6]
};

static uint16_t out_shadow; // last applied packed state (bit i = pin state)

// Drive only the pins whose bit changed since last apply (less switching noise).
static void apply_outputs(uint16_t packed) {
  uint16_t diff = packed ^ out_shadow;
  if (!diff) return;
...
static volatile uint16_t rx_pending; // newest decoded packed state
  ...
  switch (st) {
    ...
    case 4:
      if (d == (uint8_t)(b0 ^ b1 ^ b2)) {
        rx_pending = (uint16_t)(b0 & 0x7f) | ((uint16_t)(b1 & 0x7f) << 7);
        rx_ready = 1;
      }
      st = 0;
      break;
...
static void gpio_init(void) {
  ...
  // 14 panel outputs: push-pull, 2 MHz slew (low EMI), start low.
  for (uint8_t i = 0; i < 14; i++) {
  ...
  // Reserved-for-now pins kept as inputs with a defined pull (no floating -> no
  // noise). Enable as outputs later by moving them into outputs[]/b2.
  funPinMode(PD0, GPIO_CNF_IN_PUPD); funDigitalWrite(PD0, 1); // pull-up
  funPinMode(PC1, GPIO_CNF_IN_PUPD); funDigitalWrite(PC1, 1);
  funPinMode(PC2, GPIO_CNF_IN_PUPD); funDigitalWrite(PC2, 1);
